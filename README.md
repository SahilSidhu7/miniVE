...
`create` defaults to `ubuntu:24.04` with the minimal preset (git + curl); pass `--preset none|minimal|full|essentials` to change it (`essentials` = update + upgrade + common dev tooling). The one-line installers set the CLI up for you; it also ships as the `minive-cli-*` assets on each release, or build it from source with `cargo build --release --bin minive` in `src-tauri/` (binary lands in `src-tauri/target/release/`).
